<?php
    ...
    if($app::$user instanceof CouchUser){
        ...
        for($i=0;$i<count($post['athlets']);++$i){
            ...
            if($i!=0)$query.=",";
            $status="NULL";
            if($temp['status']==1)$status='true';else if($temp['status']==2)$status='false';
            $query.=sprintf($pattern,$temp['surname'],$temp['name'],$temp['patronymic'],$post['tournament'],$app::$user->id,$app::$user->getPersonInfo()['club'],$temp['gender']['value'],$temp['weight'],$temp['rank']['value'],date("Y.m.d",strtotime($temp['dateBirthday'])),$status);
        }
        //echo $query;
        $app::$connection->query($query.";");
        echo "{\"result\":\"success\"}";
